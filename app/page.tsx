...
export async function generateMetadata({ searchParams }: { searchParams: Promise<SP> }): Promise<Metadata> {
  ...
  const q = `t=${init.ticker}&m=${init.amount}${init.lump > 0 ? `&i=${init.lump}` : ""}&d=${init.buyDay}&g=${init.target}`;
  const title = `${tickerName(init.ticker)}(${init.ticker})에${init.lump > 0 ? ` ${init.lump}만원으로 시작해` : ""} 매달 ${init.amount}만원 모았다면 — ${init.target}억까지?`;
  const img = `/api/og?${q}`;
  return {
    title,
